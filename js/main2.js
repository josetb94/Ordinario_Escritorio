...
$(function() {
  ...
  function CargarTODOmenu(){
   ...
   function CargarPEDIDOS(){
    return $.ajax({
      url: 'http://ipm.esy.es/public/pedidov',
      type: 'GET',
      dataType: 'json'
    });
  ...
  function CargaHISTORIAL(){
    return $.ajax({
      url: 'http://ipm.esy.es/public/historial',
      type: 'GET',
      dataType: 'json'
    });
  ...
  function CambiarEstado(cambio){
    return $.ajax({
      url: 'http://ipm.esy.es/public/actualizarestatus',
      type: 'PUT',
      dataType: 'json',
      data: cambio
    });
  ...
  ViewModel = function(){
    ...
    _self.lista_pedidos= ko.observableArray([]);
    _self.pedidos_historial= ko.observableArray([]);

    _self.cambio=ko.observable("");
        ...
        CargaHISTORIAL().done(function(res){
          if(res.error){
            console.log(res.msg);
            return false;
          }
          ...
          //localStorage.setItem("Categoria", res.datos.ID);
          //NO SE QUE ONDA CON LA DISPONIBILIDAD DEBERIA SER VALIDADO EN EL API
          _self.pedidos_historial(res.datos);
        });
    ...
    // }
     ...
     _self.CargarPedidos = function(){
      
        CargarPEDIDOS().done(function(res){
          _self.lista_pedidos(res.datos);
        });
    }


    // _self.CambioEstado = function(){
    ...
    //     CambiarEstado(datos).done(function(res){
    //        if(res.error){
    //         console.log(res.msg);
    //         return false;
    //       }else{
    ...
    // _self.NombrePedido = function(datos){
    //   idpedido= datos.ID;
    // }
